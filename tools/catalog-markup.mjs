...
function sourceBadge(car, locale) {
  if (car.auctionSource === "Arval") return `<span class="badge source source-text">${t(locale, "vehicle.sourceBadge.arval")}</span>`;
  if (car.auctionSource === "Automotive Trade Center") return `<span class="badge source source-text">${t(locale, "vehicle.sourceBadge.atc")}</span>`;
  if (car.auctionSource === "mobile.de") return `<span class="badge source source-text">${t(locale, "vehicle.sourceBadge.mobilede")}</span>`;
  return "";
}
...
export function catalogueCards(locale) {
  ...
  return [...cars].sort((a, b) => (order[a.availability] ?? 99) - (order[b.availability] ?? 99)).map((car) => {
    ...
    return `<article class="car-card" data-status="${state}"><a class="car-card-link" href="${carRoute(locale, car.slug)}" aria-label="${esc(t(locale, "action.viewVehicle"))}: ${esc(name)}">
      <div class="car-media"><img src="/${car.mainImage.replace(/^(\.\.\/)+/, "")}" alt="${esc(t(locale, "vehicle.gallery.mainAlt", { vehicle: name }))}" width="1280" height="960" loading="lazy"><div class="car-badges"><span class="badge${sold ? " sold" : ""}">${t(locale, statusKey(car))}</span>${sourceBadge(car, locale)}</div></div>
