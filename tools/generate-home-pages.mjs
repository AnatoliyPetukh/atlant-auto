...
function auctionSourceBadge(car, locale) {
  if (car.auctionSource === "Arval") {
    return `<span class="badge source source-text">${t(locale, "vehicle.sourceBadge.arval")}</span>`;
  }
  if (car.auctionSource === "Automotive Trade Center") {
    return `<span class="badge source source-text">${t(locale, "vehicle.sourceBadge.atc")}</span>`;
  }
  if (car.auctionSource === "mobile.de") {
    return `<span class="badge source source-text">${t(locale, "vehicle.sourceBadge.mobilede")}</span>`;
  }
  return "";
}
...
function carCards(locale) {
  ...
  return [...cars].sort((first, second) =>
    ...
    const sourceBadge = auctionSourceBadge(car, locale);
...
<body>
  <header class="topbar"><a class="brand" href="${routes[locale]}" aria-label="Atlant Auto"><span class="brand-wordmark"><img src="/assets/site/atlant-auto-wordmark.svg" alt="Atlant Auto" width="720" height="150"></span></a><button class="nav-toggle" type="button" aria-expanded="false" aria-controls="primary-navigation"><span class="nav-toggle-lines" aria-hidden="true"></span><span class="sr-only">Menu</span></button><nav class="nav" id="primary-navigation" aria-label="${t(locale, "navigation.primary.label")}"><a href="${catalogue[locale]}">${t(locale, "navigation.catalog")}</a><a href="#pricing">${t(locale, "navigation.services")}</a><a href="${process[locale]}">${t(locale, "navigation.process")}</a><a href="${about[locale]}">${t(locale, "navigation.about")}</a><a href="${contact[locale]}">${t(locale, "navigation.contact")}</a></nav><div class="language-nav" aria-label="${t(locale, "language.selector.label")}">${languageNav}</div></header>
  <main>
  ...
  <footer class="footer"><div><a class="brand footer-brand" href="${routes[locale]}" aria-label="Atlant Auto"><span class="brand-wordmark"><img src="/assets/site/atlant-auto-wordmark.svg" alt="Atlant Auto" width="720" height="150"></span></a><p>${t(locale, "footer.tagline")}</p><p>${site.legalName} · NIP ${site.nip}</p></div><address><a href="tel:+48515392420">${site.phone}</a><a href="mailto:${site.email}">${site.email}</a><span>${site.vehicleLotAddress}</span><a href="${about[locale]}">${t(locale, "footer.companyInfo")}</a><a href="${faqRoute[locale]}">${t(locale, "navigation.faq")}</a><a href="${privacy[locale]}">${t(locale, "footer.privacy")}</a></address></footer>
  ...
  <script src="/app.js?v=20260818-2" defer></script><script src="/js/site-navigation.js?v=20260818-1" defer></script><script src="/js/cookie-consent.js?v=20260719-2" defer></script>
</body></html>`;
}
