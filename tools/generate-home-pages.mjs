...
function html(locale) {
  ...
  const faqColumns = [[1,3,5,7], [2,4,6,8]].map((indexes, column) => `<div class="faq-column">${indexes.map((index) => `<details${column === 0 && index === 1 ? " open" : ""}><summary>${t(locale, `home.faq.q${index}`)}</summary><p>${t(locale, `home.faq.a${index}`)}</p></details>`).join("")}</div>`).join("");
...
<body>
  ...
  <main>
    ...
    <section class="section faq-section" id="faq"><header class="faq-section-head"><div><p class="eyebrow">FAQ</p><h2>${t(locale, "home.faq.title")}</h2><p>${t(locale, "home.faq.intro")}</p></div><a class="text-link" href="${faqRoute[locale]}">${t(locale, "home.faq.more")} →</a></header><div class="faq-grid">${faqColumns}</div></section>
